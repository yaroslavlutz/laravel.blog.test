            ...
            <div class="col-sm-6">
                ...
                @foreach($articles as $article)
                    <a class="list-group-item" href="{{ route('admin_article_edit', ['id' => $article->id]) }}">
                        ...
                        </p> <!--где `pluck()` метод кот.возвращает зн-е для ключа `title`,т.е.это наименование нашей Категории и `pluck()` возвращает именно массив,то название Категорий,если их несколько
                                 для данной Article-записи, мы будем выводить через запятую и для этого мы используем метод разбивки массива - `implode()`
                             -->
                    </a>
                @endforeach
            </div>
